<?php
  class Anuncios {
    ...
    public function editAnuncio($titulo, $categoria, $valor, $descricao, $estado, $fotos, $id){
      ...
      //correção de codigo para php maior que a versão 7.2
      $pkCount = (is_array($fotos) ? count($fotos) : 0);

      //verifica se $fotos possui algum conteudo enviado
      if($pkCount > 0) {
        for($q=0; $q<count($fotos['tmp_name']); $q++) {
            ...
            if(in_array($tipo, array('image/jpeg', 'image/png'))) {
              //salvar como jpg e fazer um hash do momento atual com um random de 0 - 9999
              $tmpname = md5(time().rand(0,9999)).'.jpg';
              // movimentar a foto pra o diretorio anuncios
              move_uploaded_file($fotos['tmp_name'][$q], 'assets/images/anuncios/'.$tmpname);
              //receber o tamanho da foto original
              ...
              // gerar tamanho proporcional a foto.
              ...
              //inserir tamanho original dentro da nova imagem com o tamanho correto
              imagecopyresampled($img, $origi, 0, 0, 0, 0, $width, $height, $width_orig, $height_orig);
              // salvar a imagem na pasta assets
              imagejpeg($img, 'assets/images/anuncios/'.$tmpname, 80);

              //faz o relacionamento da imagem salva no BD
